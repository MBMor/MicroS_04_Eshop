...
| `GAP-004` Closed — TECH-08 accepted | `R-RESILIENCE-001`; all seven HTTP processes expose `/live` and `/ready`. Catalog/PostgreSQL and Basket/Redis directly prove healthy, outage and no-restart recovery. PR #15 merged as `36f6c5d`; Main CI #66/R92 passed 24/24 and Release #4/R95 passed 9/9, including C80/`ESHOP-RESILIENCE-002`. | Oracle Approved below; `CTRL-OPS-READINESS-001` Implemented/Direct for owned PostgreSQL and Redis variants; `GATE-OPS-001` remains a Future W3 activation prerequisite. | No remaining GAP-004 action. Retain full Compose/delayed-dependency and orchestration-consumption evidence under GAP-013. | Complete; Main + Release evidence accepted. |
...
| `GAP-026` Closed — TECH-05 accepted | `R-GW-AUTH-001`; the authoritative registry covers all 13 YARP proxy routes and 5 local endpoints, including TECH-08 liveness/readiness. One 45-row theory asserts public/authenticated/role behavior and zero downstream requests after denials; the original boundary passed Main CI #52/R79 and the health additions passed Main CI #66/R92 plus Release #4/R95. | Oracle Approved; `CTRL-SEC-GATEWAY-001` is Direct for the gateway boundary; `GATE-SEC-001` remains a Future W2 activation prerequisite. | No remaining GAP-026 implementation action. Retain the fail-closed registry and matrix. Catalog service authorization is accepted under GAP-003; deployable topology remains GAP-013. | Complete; Main + Release evidence accepted. |
...
The accepted TECH-08 baseline narrows runtime by event without narrowing total governed coverage: PR 77, Main primary 102 and cumulative Main 179, Nightly 19 and Release overlap 17 selectors. Main publication is locked to `12/24/3/4` and Release to 9 aggregate TestIntents over 26 binding edges. QA-04/TECH-06 still requires every upstream job to succeed and validates the complete report set before the first remote call. Main run `30486673945` and governed Release `30487730431` passed and published the expected TestRail results.
...
**Accepted evidence:** PR #15 merged TECH-08 as `36f6c5d`. Main [`30486673945`](https://github.com/MBMor/MicroS_04_Eshop/actions/runs/30486673945) completed all quality, build, container, E2E and TestRail publication jobs; TestRail CI #66/R92 closed 24/24 Passed and contains Passed C80/`ESHOP-RESILIENCE-002`. Governed Release [`30487730431`](https://github.com/MBMor/MicroS_04_Eshop/actions/runs/30487730431) completed all six backend partitions and publication; Release #4/R95 closed 9/9 Passed and C80 passed again. GAP-004 is closed without activating or evaluating `GATE-OPS-001`; full Compose/delayed-dependency behavior remains GAP-013.
...
**Accepted evidence:** Catalog 19/19 and API Gateway 68/68 passed locally. The initial Main publication exposed only the incompatible manifest-version bump; hotfix `4ec560f` restored version 1 and added a production-manifest regression test. Main [`CI #62`](https://github.com/MBMor/MicroS_04_Eshop/actions/runs/30458083498) passed and published closed R86–R89 at `12/23/3/4`. Governed Release [`30481512624`](https://github.com/MBMor/MicroS_04_Eshop/actions/runs/30481512624) published closed R90 with 8/8 Passed, including `ESHOP-CATALOG-001` and `ESHOP-GW-001`. GAP-003 is closed without activating or evaluating `GATE-SEC-001`.

## Change log

| Version | Date | Material change | Approved by |
|---|---|---|---|
| 3.1 | 2026-07-29 | Closed GAP-004 after PR #15, Main `36f6c5d`/CI #66/R92 and governed Release #4/R95 accepted QA-05/TECH-08. | Pending review |
| 3.0 | 2026-07-29 | Groomed and implemented QA-05/TECH-08/GAP-004 locally with seven-service liveness/readiness and direct PostgreSQL/Redis outage/recovery evidence; shared acceptance remained pending. | Pending review |
| 2.9 | 2026-07-29 | Closed GAP-003 after hotfix `4ec560f`, Main CI #62/R87 and governed Release R90 accepted TECH-07. | Pending review |
