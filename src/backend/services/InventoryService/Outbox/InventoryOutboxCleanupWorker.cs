...
public sealed class InventoryOutboxCleanupWorker(
    IServiceScopeFactory scopeFactory,
    TimeProvider timeProvider,
    IOptions<OutboxProcessingOptions> options,
    ILogger<InventoryOutboxCleanupWorker> logger)
    : BackgroundService
{
    private static readonly Action<ILogger, int, Exception?>
        LogMessagesDeleted =
            LoggerMessage.Define<int>(
                ...
                new EventId(2150, nameof(LogMessagesDeleted)),
                "Deleted {DeletedCount} published inventory outbox messages.");

    private static readonly Action<ILogger, Exception?>
        LogCleanupFailed =
            LoggerMessage.Define(
                LogLevel.Error,
                new EventId(2151, nameof(LogCleanupFailed)),
                "Inventory outbox cleanup failed.");
