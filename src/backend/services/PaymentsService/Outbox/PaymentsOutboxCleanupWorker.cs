...
public sealed class PaymentsOutboxCleanupWorker(
    IServiceScopeFactory scopeFactory,
    TimeProvider timeProvider,
    IOptions<OutboxProcessingOptions> options,
    ILogger<PaymentsOutboxCleanupWorker> logger)
    : BackgroundService
{
    private static readonly Action<ILogger, int, Exception?>
        LogMessagesDeleted =
            LoggerMessage.Define<int>(
                ...
                new EventId(3150, nameof(LogMessagesDeleted)),
                "Deleted {DeletedCount} published payments outbox messages.");

    private static readonly Action<ILogger, Exception?>
        LogCleanupFailed =
            LoggerMessage.Define(
                LogLevel.Error,
                new EventId(3151, nameof(LogCleanupFailed)),
                "Payments outbox cleanup failed.");
