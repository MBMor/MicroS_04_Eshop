    ...
    : BackgroundService
{
    ...
    private static readonly Action<ILogger, ulong, Exception?>
        LogInvalidJson =
            LoggerMessage.Define<ulong>(
                LogLevel.Error,
                new EventId(3700, nameof(LogInvalidJson)),
    ...
    private static readonly Action<ILogger, ulong, Exception?>
        LogProcessingFailed =
            LoggerMessage.Define<ulong>(
                LogLevel.Error,
                new EventId(3701, nameof(LogProcessingFailed)),
    ...
    private static readonly Action<ILogger, ulong, Exception?>
        LogPermanentFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Error,
                new EventId(3702, nameof(LogPermanentFailure)),
                "StockReleased message {DeliveryTag} failed with a permanent processing error and will be dead-lettered.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogTransientFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Warning,
                new EventId(3703, nameof(LogTransientFailure)),
                "StockReleased message {DeliveryTag} failed with a transient processing error and will be retried.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogDuplicateMessage =
            LoggerMessage.Define<ulong>(
                LogLevel.Information,
                new EventId(3704, nameof(LogDuplicateMessage)),
                "StockReleased message {DeliveryTag} was already processed by another consumer instance.");
        ...
        try
        {
            MessageEnvelope<StockReleasedV1> envelope =
                serializer.Deserialize<
                    MessageEnvelope<StockReleasedV1>>(
                    delivery.Body.Span);
