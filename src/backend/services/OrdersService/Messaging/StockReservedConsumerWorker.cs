    ...
    : BackgroundService
{
    ...
    private static readonly Action<ILogger, ulong, Exception?>
        LogPermanentFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Error,
                new EventId(2202, nameof(LogPermanentFailure)),
                "StockReserved message {DeliveryTag} failed with a permanent processing error and will be dead-lettered.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogTransientFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Warning,
                new EventId(2203, nameof(LogTransientFailure)),
                "StockReserved message {DeliveryTag} failed with a transient processing error and will be retried.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogDuplicateMessage =
            LoggerMessage.Define<ulong>(
                LogLevel.Information,
                new EventId(2204, nameof(LogDuplicateMessage)),
                "StockReserved message {DeliveryTag} was already processed by another consumer instance.");
        ...
        try
        {
            MessageEnvelope<StockReservedV1> envelope =
                serializer.Deserialize<
                    MessageEnvelope<StockReservedV1>>(
                    delivery.Body.Span);
