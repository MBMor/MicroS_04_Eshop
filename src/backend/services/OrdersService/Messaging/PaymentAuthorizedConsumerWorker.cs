    ...
    : BackgroundService
{
    ...
    private static readonly Action<ILogger, ulong, Exception?>
        LogPermanentFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Error,
                new EventId(3502, nameof(LogPermanentFailure)),
                "PaymentAuthorized message {DeliveryTag} failed with a permanent processing error and will be dead-lettered.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogTransientFailure =
            LoggerMessage.Define<ulong>(
                LogLevel.Warning,
                new EventId(3503, nameof(LogTransientFailure)),
                "PaymentAuthorized message {DeliveryTag} failed with a transient processing error and will be retried.");

    private static readonly Action<ILogger, ulong, Exception?>
        LogDuplicateMessage =
            LoggerMessage.Define<ulong>(
                LogLevel.Information,
                new EventId(3504, nameof(LogDuplicateMessage)),
                "PaymentAuthorized message {DeliveryTag} was already processed by another consumer instance.");
        ...
        try
        {
            MessageEnvelope<PaymentAuthorizedV1> envelope =
                serializer.Deserialize<
                    MessageEnvelope<PaymentAuthorizedV1>>(
                    delivery.Body.Span);
