    ...
    : BackgroundService
{
    ...
    private static readonly Action<ILogger, Guid, string, Exception?>
        LogClaimLost =
            LoggerMessage.Define<Guid, string>(
                LogLevel.Warning,
                new EventId(2402, nameof(LogClaimLost)),
                "Orders outbox message {MessageId} is no longer claimed by worker {WorkerId}.");

    private static readonly Action<ILogger, Guid, Exception?>
        LogStateUpdateFailed =
            LoggerMessage.Define<Guid>(
                LogLevel.Error,
                new EventId(2403, nameof(LogStateUpdateFailed)),
                "Updating state of orders outbox message {MessageId} failed.");
            ...
            CancellationToken cancellationToken)
    {
        ...
        OrdersOutboxStore store =
            scope.ServiceProvider
                .GetRequiredService<OrdersOutboxStore>();
        ...
        CancellationToken cancellationToken)
    {
        ...
        return message.RoutingKey switch
        {
            ...
            RabbitMqRoutingKeys.PaymentRequestedV1 =>
                eventPublisher.PublishAsync(
                    Deserialize<PaymentRequestedV1>(
                        message.Payload),
                    message.RoutingKey,
                    context,
                    cancellationToken),
            ...
            RabbitMqRoutingKeys.StockReleaseRequestedV1 =>
                eventPublisher.PublishAsync(
                    Deserialize<StockReleaseRequestedV1>(
                        message.Payload),
                    message.RoutingKey,
                    context,
                    cancellationToken),
            ...
            _ => throw new InvalidOperationException(
                $"Unsupported orders outbox routing key " +
                $"'{message.RoutingKey}'.")
        };
    }
